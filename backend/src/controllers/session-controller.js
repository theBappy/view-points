...
export async function createSession(req, res) {
  try {
    ...
    if (!problem || !difficulty) {
      return res
        .status(400)
        .json({ msg: "Problem & difficulty are required." });
    }
...
export async function joinSession(req, res) {
  try {
    ...
    if (session.participant) {
      return res.status(404).json({ msg: "Session is already full" });
    }
...
export async function endSession(req, res) {
  try {
    ...
    //check if user is the host or not, bkz only host can end the session
